...
function QueryProperties(sortFormElement, filterFormElement, propertiesRowElement) {
...
QueryProperties.prototype.drawProperties = function (properties) {
    function addCaracteristici(caracteristici) {
        var limit = 4;
        var caractNo = 0;
        var caracteristici_elements = "";
        for (var c in caracteristici) {
            if (caractNo === limit) break;
            var list_item = `
            <li>
                <span> ${caracteristici[c]['key']} </span> ${caracteristici[c]['value']}
            </li>`
            caractNo++;
            caracteristici_elements += list_item;
        }
        return caracteristici_elements;
    }
            ...
            <div class="col-md-12">
                <div class="property_list_1 property_item bg-white mb_30">
                    <div class="zoom_effect_1"> 
                     <a href="/${createPropertyPath(property.status)}/${property._id}">
                        <img src="/${property.thumbnail}" alt="Image Found">
                     </a>
                     <div class="upper_2">
                                                <i class="far fa-images"></i>
                                                <span>${property.imagini.length}</span>
                                            </div>
                    </div>
                        <div class="float-right property_list_details p-3">
                            <div class="property_text">
                                <h5 class="title">
                                    <a href="/${createPropertyPath(property.status)}/${property._id}">${property.titlu}</a>
                                </h5>
                        ...
                        <div class="quantity">
                                    <ul>
                                        ${addCaracteristici(property.caracteristici)}
                                    </ul>
                                </div>
                            </div>
                            <div class="bed_area d-table w-100">
                                <ul>
                                    ...
                                    <li><a href="/${createPropertyPath(property.status)}/${property._id}">
                                                            <button type="button"
